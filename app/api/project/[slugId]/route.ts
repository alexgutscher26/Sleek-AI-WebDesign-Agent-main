...
export async function GET(req: NextRequest, { params }: { params: Promise<{ slugId: string }> }) {
  try {
    ...
    const { data: project, error } = await getOwnedProjectBySlug<{
      ...
      metadata?: unknown
      updatedAt?: string
    }>(insforge, user.id, slugId, "id, title, metadata, updatedAt")
    ...
    const { data: messages } = await insforge.database
      .from<ProjectMessageRecord[]>("messages")
      .select("*")
